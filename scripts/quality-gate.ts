...
const automatedChecks: Record<string, () => { success: boolean; output: string }> = {
    ...
    "supabase db reset": () =>
        runShellCheck(
            'npx supabase db reset --linked 2>&1 || echo "Supabase not linked — skipping"'
        ),
    ...
    'grep -rn "style={{" src/': () => {
        ...
        return {
            ...
            output: lines.length > 0 ? `Found ${lines.length} inline styles` : "No inline styles",
        };
    },
...
function evaluateCriterion(
    ...
    if (waiver) {
        return {
            ...base,
            status: "WAIVED",
            message: `Waived until ${waiver.expiresAt}: ${waiver.justification}`,
            waiverInfo: waiver,
        };
    }
    ...
    if (criterion.checkType === "automated" && criterion.automatedCheck) {
        ...
        return {
            ...
            message: `No automated runner for: ${criterion.automatedCheck}`,
        };
    }
    ...
    if (criterion.checkType === "semi-automated" && criterion.automatedCheck) {
        ...
        if (attestation) {
            return {
                ...base,
                status: "PASS",
                message: `Automated check passed + attested by ${attestation.attestedBy}`,
            };
        }
        return {
            ...base,
            status: "NEEDS_ATTESTATION",
            message: "Automated check passed but human attestation required",
        };
    }
    ...
    if (criterion.checkType === "manual" || criterion.checkType === "semi-automated") {
        ...
        if (attestation) {
            return {
                ...base,
                status: "PASS",
                message: `Attested by ${attestation.attestedBy} on ${attestation.attestedAt}`,
            };
        }
    ...
    if (criterion.checkType === "continuous") {
        ...
        if (attestation) {
            return {
                ...base,
                status: "PASS",
                message: `Monitoring confirmed by ${attestation.attestedBy}`,
            };
        }
        return {
            ...base,
            status: "NEEDS_ATTESTATION",
            message: "Requires monitoring confirmation",
        };
    }
...
async function main(): Promise<void> {
    ...
    coreChecks.push({
        criterionId: "CORE-LINT",
        title: "ESLint zero warnings",
        section: 12,
        category: "code-quality",
        severity: "CRITICAL",
        checkType: "automated",
    ...
    const hasHighVulns =
        auditResult.output.includes("high") || auditResult.output.includes("critical");
    coreChecks.push({
        criterionId: "CORE-AUDIT",
    ...
    const blockers = allResults.filter(
        (r) => r.status === "FAIL" || r.status === "NEEDS_ATTESTATION"
    );
