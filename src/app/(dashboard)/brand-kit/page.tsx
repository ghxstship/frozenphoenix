                    ...
                    <CardContent className="space-y-4">
                        {wizardStep === 0 && (
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground mt-2">
                                    The brand kit will be linked to this client across all projects.
                                </p>
                            </div>
                        )}
                        {wizardStep === 1 && (
                        ...
                        {wizardStep === 2 && (
                            <div>
                                ...
                                <Input
                                    ...
                                    onChange={(e) =>
                                        setWizardData({ ...wizardData, fontFamily: e.target.value })
                                    }
                                />
                                <div className="mt-3 p-4 rounded-lg bg-secondary/30">
                                    <p
                                        className="text-xl font-bold"
                                        style={{ fontFamily: wizardData.fontFamily || "inherit" }}
                                    >
                        ...
                        <div className="flex justify-between pt-2">
                            ...
                            {wizardStep < wizardSteps.length - 1 ? (
                                ...
                                <Button
                                    disabled={!wizardData.clientName || createBrandKit.isPending}
                                    onClick={() =>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex gap-2">
                                            <Button
                                                ...
                                                onClick={() =>
                                                    router.push(`/brand-kit/${kit.id}?tab=assets`)
                                                }
                                            >
...
export default function BrandKitPage() {
    ...
    const brandKits: BrandKitView[] = useMemo(
        () =>
            (sbBrandKits ?? []).map((kit) => ({
                id: kit.id,
    ...
    const linkedProjectCount = projects.filter((p) =>
        brandKits.some((k) => p.client.toLowerCase().includes(k.clientName.toLowerCase()))
    ).length;
    ...
    const config: ListPageConfig = useMemo(
        () => ({
            ...
            stats: [
                { label: "Brand Kits", icon: Palette, compute: () => brandKits.length },
                {
                    label: "With Logo",
                    icon: ImageIcon,
                    compute: () => brandKits.filter((k) => k.logoUrl).length,
                },
                {
                    label: "Font Families",
                    icon: Type,
                    compute: () => [...new Set(brandKits.map((k) => k.fontFamily))].length,
                },
                { label: "Linked Projects", icon: FileText, compute: () => linkedProjectCount },
            ],
            contentSlot: (
                <BrandKitContent
                    brandKits={brandKits}
        ...
        [brandKits, projects, linkedProjectCount]
    );

    return (
        <ListPageShell
            config={config}
            data={brandKits as unknown as Record<string, unknown>[]}
            isLoading={isLoading}
        />
    );
}
