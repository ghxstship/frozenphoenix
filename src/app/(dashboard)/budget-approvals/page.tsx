    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <StatCard title="Total Approved" value={formatCurrency(totalApproved)} icon={ShieldCheck} />
            </div>
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="text-base flex items-center gap-2"><ShieldCheck className="h-4 w-4" /> Budget Approvals ({filtered.length})</CardTitle>
                </CardHeader>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-border bg-muted/50">
                                    ...
                                    <th className="text-left p-3 font-medium">Threshold Rule</th>
                            ...
                            <tbody>
                                {filtered.map(a => (
                                    <tr key={a.id} className="border-b border-border hover:bg-muted/30 transition-colors cursor-pointer">
                                        <td className="p-3">
                                            <div className="font-medium text-xs">{a.entity_type}</div>
                                            <div className="text-[10px] text-muted-foreground">{a.entity_id}</div>
                                        </td>
                                        ...
                                        <td className="p-3 text-xs">Level {a.approval_level}</td>
                                        ...
                                        <td className="p-3 text-xs">{a.approved_at ? new Date(a.approved_at).toLocaleDateString() : "—"}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
