                        ...
                        >
                            <div className="min-w-0 flex-1">
                                ...
                                <p className="text-xs text-muted-foreground">
                                    {c.expiry_date
                                        ? `Expires ${String(formatDate(c.expiry_date, "compact"))}`
                                        : "No expiry"}
                                </p>
                            </div>
                        </div>
...
export function CertificationDetailClient({
    ...
    const rec = (cert ?? initialRecord) as Record<string, unknown> | null;
    ...
    const config: DetailPageConfig = {
        ...
        stats: [
            { label: "Status", icon: BadgeCheck, compute: () => status.replace(/_/g, " ") },
            {
                label: "Issued",
                icon: Calendar,
                compute: () => (issuedDate ? String(formatDate(issuedDate, "compact")) : "—"),
            },
            {
                label: "Blocks Usage",
        ...
        tabs: [
            { id: "hr-certs", label: "HR Certs", content: <HrCertificationsTab /> },
            { id: "user-certs", label: "User Certs", content: <UserCertificationsTab /> },
            {
                id: "documents",
                label: "Documents",
                content: (
            ...
            menuItems={[
                {
                    ...
                    onClick: () => router.push(`/certifications/${id}/edit`),
                },
                {
                    label: "Upload Document",
                    onClick: () =>
                        router.push(`/documents/new?entityType=certification&entityId=${id}`),
                },
                ...crudMenuItems,
            ]}
            avatar={
