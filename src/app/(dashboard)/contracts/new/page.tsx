...
export default function NewContractPage() {
    ...
    const canNext = currentStep === "type" ? !!selectedType : currentStep === "details" ? !!formData.title && !!formData.counterpartyName : true;
    ...
    return (
        <div className="space-y-6 animate-fade-in max-w-3xl mx-auto">
            <PageHeader title="New Contract" description="Create a new contract, NDA, SOW, or amendment">
                <Link href="/contracts">
                    <Button variant="outline" size="sm"><ArrowLeft className="mr-2 h-4 w-4" />Cancel</Button>
                </Link>
            </PageHeader>
                ...
                {STEPS.map((step, i) => {
                    ...
                    return (
                        <div key={step.key} className="flex items-center gap-2 flex-1">
                            <div className={`h-8 w-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-colors ${isComplete ? "bg-success text-success-foreground" : isActive ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
                            ...
                            {i < STEPS.length - 1 && <div className={`flex-1 h-0.5 ${isComplete ? "bg-success" : "bg-muted"}`} />}
                        </div>
                    );
                })}
            </div>
                        ...
                        >
                            <CardContent className="py-4">
                                <div className="flex items-center gap-3">
                                    <div className={`h-10 w-10 rounded-xl flex items-center justify-center ${selectedType === ct.value ? "bg-primary/10" : "bg-secondary/50"}`}>
                                        <FileSignature className={`h-5 w-5 ${selectedType === ct.value ? "text-primary" : "text-muted-foreground"}`} />
                                    </div>
                                    <div>
                                        <p className="text-sm font-semibold">{ct.label}</p>
                                        <Badge variant={ct.variant} className="mt-1">{ct.value}</Badge>
                                    </div>
                                </div>
                            </CardContent>
                        </Card>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <div>
                            ...
                            <Input placeholder="e.g., Nike Master Services Agreement" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} />
                        </div>
                        <div>
                            <label className="text-sm font-medium mb-1 block">Counterparty Name</label>
                            <Input placeholder="e.g., Nike" value={formData.counterpartyName} onChange={(e) => setFormData({ ...formData, counterpartyName: e.target.value })} />
                        </div>
                        <div>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            />
                            <label htmlFor="autoRenew" className="text-sm font-medium">Auto-renew upon expiration</label>
                        </div>
                    </CardContent>
                </Card>
            )}
            ...
            {currentStep === "review" && (
                <Card>
                    <CardHeader><CardTitle className="text-base">Review Contract</CardTitle></CardHeader>
                    <CardContent>
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="p-3 rounded-lg bg-secondary/30">
                                    <p className="text-xs text-muted-foreground">Type</p>
                                    <p className="text-sm font-semibold">{CONTRACT_TYPES.find(t => t.value === selectedType)?.label}</p>
                                </div>
                                <div className="p-3 rounded-lg bg-secondary/30">
                                    <p className="text-xs text-muted-foreground">Counterparty</p>
                                    <p className="text-sm font-semibold">{formData.counterpartyName || "—"}</p>
                                </div>
                                <div className="p-3 rounded-lg bg-secondary/30">
                                    <p className="text-xs text-muted-foreground">Value</p>
                                    <p className="text-sm font-semibold">{formData.value ? `$${Number(formData.value).toLocaleString()}` : "—"}</p>
                                </div>
                                <div className="p-3 rounded-lg bg-secondary/30">
                                    <p className="text-xs text-muted-foreground">Period</p>
                                    <p className="text-sm font-semibold">{formData.effectiveDate || "—"} → {formData.expirationDate || "—"}</p>
                                </div>
                            </div>
                            <div className="p-3 rounded-lg bg-secondary/30">
