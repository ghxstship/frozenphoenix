...
import {
    FileSignature, Plus, Search, Send, Eye, CheckCircle2,
...
const ESTIMATE_STATUSES: EstimateStatus[] = ["draft", "sent", "viewed", "accepted", "rejected", "expired", "converted"];
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard title="Total Pipeline" value={formatCurrency(totalValue)} icon={DollarSign} />
                <StatCard title="Accepted Value" value={formatCurrency(acceptedValue)} icon={CheckCircle2} />
                <StatCard title="Pending Response" value={pendingCount} icon={Clock} />
                <StatCard title="Total Estimates" value={estimates.length} icon={FileSignature} />
            </div>
            ...
            {viewMode === "cards" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filtered.map((est, i) => (
                        <Card key={est.id} className="animate-slide-up hover:shadow-md transition-shadow cursor-pointer" style={{ animationDelay: `${i * 60}ms` }}>
                            <CardContent className="pt-4">
                                <div className="flex items-start justify-between mb-2">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <h3 className="text-sm font-bold truncate">{est.title}</h3>
                                    </div>
                                    <StatusBadge status={est.status} className="text-[10px] ml-2 shrink-0" />
                                </div>

                                {est.companyName && (
                                    <p className="text-xs text-muted-foreground mb-1">{est.companyName}</p>
                                )}
                                {est.contactName && (
                                    <p className="text-[10px] text-muted-foreground mb-3">{est.contactName}</p>
                                )}

                                <div className="flex items-center justify-between mb-3">
                                    <span className="text-lg font-bold">{formatCurrency(est.total)}</span>
                                    {est.discountPercent > 0 && (
                                        <span className="text-[10px] text-muted-foreground">{est.discountPercent}% discount</span>
                                    )}
                                </div>
                                ...
                                <div className="text-xs text-muted-foreground space-y-1">
                                    <div className="flex items-center justify-between">
                                        ...
                                        {est.lineItems.some(li => li.optional) && (
                                            <span>{est.lineItems.filter(li => li.optional).length} optional</span>
                                        )}
                                    </div>
                                    {est.validUntil && (
                                ...
                                <div className="flex items-center gap-2 mt-3 pt-2 border-t border-border">
                                    {est.sentAt && (
                                        <span className="text-[10px] text-muted-foreground flex items-center gap-1"><Send className="h-3 w-3" /> Sent</span>
                                    )}
                                    {est.viewedAt && (
                                        <span className="text-[10px] text-muted-foreground flex items-center gap-1"><Eye className="h-3 w-3" /> Viewed</span>
                                    )}
                                    {est.signedBy && (
                                        <span className="text-[10px] text-success flex items-center gap-1"><CheckCircle2 className="h-3 w-3" /> Signed by {est.signedBy}</span>
                                    )}
                                </div>
                            </CardContent>
                        </Card>
                    ...
                    <CardContent className="p-0">
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {filtered.map(est => (
                                        <tr key={est.id} className="border-b border-border hover:bg-muted/30 transition-colors cursor-pointer">
                                            <td className="p-3 font-mono text-xs">{est.number}</td>
                                            <td className="p-3 font-medium">{est.title}</td>
                                            <td className="p-3 text-muted-foreground">{est.companyName || "—"}</td>
                                            <td className="p-3"><StatusBadge status={est.status} className="text-[10px]" /></td>
                                            <td className="p-3 text-right font-medium">{formatCurrency(est.total)}</td>
                                            <td className="p-3 text-xs text-muted-foreground">{est.validUntil || "—"}</td>
                                            <td className="p-3 text-xs text-muted-foreground">{new Date(est.createdAt).toLocaleDateString()}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
