...
export function IncidentDetailClient({
    ...
    const { data: sbActivity } = useRecordActivityLog("incident", id);
    const { data: sbComments } = useRecordComments("incident", id);
    ...
    const inc = (incident ?? initialRecord) as Record<string, unknown> | null;
        ...
        <div className="density-gap-page">
            ...
            {incident.immediate_actions && (
                <Card>
                    ...
                    <CardContent>
                        <p className="text-sm text-muted-foreground whitespace-pre-wrap">
                            {String(incident.immediate_actions)}
                        </p>
                    </CardContent>
                </Card>
            )}
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="flex items-center gap-2 text-sm">
                            ...
                            <span>{String(incident.specific_location)}</span>
                        </div>
                        {location && (
    ...
    const config: DetailPageConfig = {
        ...BASE_CONFIG,
        subtitleFn: () =>
            incident ? `${String(incident.number)} · ${String(incident.specific_location)}` : "",
        sidebarSlot,
        overviewSlot,
        stats: [
                        ...
                        <Card>
                            ...
                            <CardContent>
                                {incident.preventive_measures ? (
                                    <p className="text-sm text-muted-foreground whitespace-pre-wrap">
                                        {String(incident.preventive_measures)}
                                    </p>
                                ) : (
    ...
    return (
        <DetailPageShell
            ...
            menuItems={[
                {
                    ...
                    onClick: () => router.push(`/insurance-policies/new?incidentId=${id}`),
                },
            ...
            actions={
                <Button onClick={() => router.push(`/incidents/${id}/edit`)}>
