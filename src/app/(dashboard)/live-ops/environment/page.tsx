...
const mockReadings: MockReading[] = [
    {
        ...
        windGustsMph: 14,
        noiseLevelDb: 92,
        noiseLocation: "Main Stage FOH",
        totalPowerLoadAmps: 680,
        powerCapacityAmps: 800,
        generatorFuelPercent: 78,
        weatherAlert: undefined,
    },
    {
        id: "2",
        recordedAt: "2026-02-24T17:30:00Z",
        temperatureF: 73,
        humidityPercent: 44,
        windSpeedMph: 10,
        windGustsMph: 16,
        noiseLevelDb: 85,
        noiseLocation: "FOH Bar Area",
        totalPowerLoadAmps: 650,
        powerCapacityAmps: 800,
    ...
    {
        id: "3",
        recordedAt: "2026-02-24T17:00:00Z",
        temperatureF: 74,
        humidityPercent: 42,
        windSpeedMph: 7,
        windGustsMph: 12,
        noiseLevelDb: 78,
        noiseLocation: "VIP Lounge",
        totalPowerLoadAmps: 620,
        powerCapacityAmps: 800,
        generatorFuelPercent: 85,
        weatherAlert: undefined,
    },
];
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard
                    title="Temperature"
                    value={`${latest.temperatureF}°F`}
                    icon={Thermometer}
                />
                <StatCard
                    title="Wind"
                    value={`${latest.windSpeedMph} mph (gusts ${latest.windGustsMph})`}
                    icon={Wind}
                />
                    ...
                    <CardContent className="py-4">
                        ...
                        <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                                ...
                                <span className="font-medium">{latest.temperatureF}°F</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-muted-foreground">Humidity</span>
                                <span className="font-medium">{latest.humidityPercent}%</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-muted-foreground">Wind Speed</span>
                                <span className="font-medium">{latest.windSpeedMph} mph</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-muted-foreground">Wind Gusts</span>
                                <span className="font-medium">{latest.windGustsMph} mph</span>
                            </div>
                            <div className="flex justify-between">
                        ...
                        <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                                ...
                                <span className="font-medium">
                                    {latest.totalPowerLoadAmps}A / {latest.powerCapacityAmps}A
                                </span>
                            </div>
                            <div className="flex justify-between">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span
                                    className={`font-medium ${latest.generatorFuelPercent < 30 ? "text-destructive" : "text-success"}`}
                                >
                                    {latest.generatorFuelPercent}%
                                </span>
                            </div>
                        </div>
                        ...
                        {mockReadings.map((r, i) => (
                            <StaggerItem key={r.id} index={i} stagger="tight">
                                <div className="flex items-center gap-4 text-sm py-2 border-b border-border last:border-0">
                                    ...
                                    <span className="w-24">
                                        {r.totalPowerLoadAmps}A (
                                        {Math.round(
                                            (r.totalPowerLoadAmps / r.powerCapacityAmps) * 100
                                        )}
                                        %)
                                    </span>
