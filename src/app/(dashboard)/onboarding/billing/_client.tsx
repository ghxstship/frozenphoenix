...
function BillingForm({
    ...
    const wizardConfig: WizardConfig = useMemo(
        () => ({
            ...
            description:
                "Plans differ by capabilities — not just seats. Unlock modules, integrations, automations, AI, and customization as your team grows.",
            icon: CreditCard,
            showProgress: false,
            submitLabel: `Continue with ${selectedDisplay.name}`,
            onCancel: skipAndNavigate,
            onComplete: handleSelectPlanAndContinue,
            steps: [
                {
                    id: "billing",
                    label: "Billing",
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="flex items-center justify-center gap-3">
                                ...
                                >
                                    Annual
                                    {annualSavings > 0 && (
                                        <Badge variant="success" className="density-caption px-1.5">
                                            Save {annualSavings}%
                                        </Badge>
                                    )}
                                </button>
                                    ...
                                    return (
                                        ...
                                        >
                                            {card.recommended && (
                                                <Badge
                                                    variant="default"
                                                    className="absolute -top-2.5 left-1/2 -translate-x-1/2 density-caption"
                                                >
                                                    Most popular
                                                </Badge>
                                            )}
                                            ...
                                            <div className="space-y-3 flex-1">
                                                ...
                                                {!isFree && (
                                                    <p className="density-caption text-muted-foreground">
                                                        {entitlements.pricing.includedSeats} seats
                                                        included
                                                ...
                                                <div className="border-t pt-3 space-y-1.5">
                                                    {card.highlights.map((hl) => (
                                                        <div
                                                            key={hl}
                                                            className="flex items-start gap-1.5 density-caption"
                                                        >
                                            ...
                                            <div className="border-t pt-3 mt-3 flex flex-wrap gap-1.5">
                                                {Object.entries(DIMENSION_LABELS).map(
                                                    ([key, label]) => {
                                                        ...
                                                        const hasCapability = Object.values(
                                                            sectionObj
                                                        ).some(
                                                            (v) =>
                                                                v === true ||
                                                        ...
                                                        return (
                                                            <span
                                                                key={key}
                                                                className={cn(
                                                                    "inline-flex items-center gap-0.5 rounded px-1.5 py-0.5 density-caption",
                                                                    hasCapability
                            ...
                            <p className="text-center density-caption text-muted-foreground">
                                14-day free trial on all paid plans. No credit card required to
                                start. Payment processing will be configured in organization
                                settings.
                            </p>
                        </div>
                    ),
    ...
    return <WizardShell config={wizardConfig} isSubmitting={completing} />;
}
