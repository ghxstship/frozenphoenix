    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex items-center gap-2">
                {(Object.entries(typeConfig) as [StakeholderType, typeof typeConfig[StakeholderType]][]).map(([type, config]) => (
                    <div key={type} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-secondary/50 text-xs font-medium">
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="mt-2 space-y-0.5 text-[11px] text-muted-foreground">
                                            <p className="flex items-center gap-1"><Mail className="h-3 w-3" />{person.email}</p>
                                            {person.phone && <p className="flex items-center gap-1"><Phone className="h-3 w-3" />{person.phone}</p>}
                                        </div>
