...
export function RecurringInvoiceDetailClient({
    id,
    initialRecord,
}: {
    id: string;
    ...
    const { data: sbRecord, isLoading } = useRecurringInvoice(id);
    const ri = (sbRecord ?? initialRecord) as Record<string, unknown> | null;
                    ...
                    {nextInvoiceDate && (
                        <div className="flex justify-between">
                            ...
                            <span className="font-medium">
                                {String(formatDate(nextInvoiceDate, "compact"))}
                            </span>
                        </div>
                    )}
                </CardContent>
                ...
                <CardContent className="space-y-3 text-sm">
                    {lastInvoiceDate && (
                        <div className="flex justify-between">
                            ...
                            <span className="font-medium">
                                {String(formatDate(lastInvoiceDate, "compact"))}
                            </span>
                        </div>
                    )}
                </CardContent>
    ...
    const config: DetailPageConfig = {
        ...
        stats: [
            ...
            { label: "Generated", icon: RefreshCw, compute: () => `${invoicesGenerated} invoices` },
            {
                ...
                icon: RefreshCw,
                compute: () =>
                    nextInvoiceDate ? String(formatDate(nextInvoiceDate, "compact")) : "TBD",
            },
        ],
        overviewSlot: (
                                ...
                                {history.map((inv) => (
                                    ...
                                    >
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                {String(formatDate(inv.date, "compact"))}
                                            </p>
                                        </div>
    ...
    return (
        <DetailPageShell
            ...
            menuItems={[
                {
                    label: "Edit Schedule",
                    onClick: () => router.push(`/recurring-invoices/${id}/edit`),
                },
                ...crudMenuItems,
            ]}
            avatar={
            ...
            actions={
                <div className="flex gap-2">
                    ...
                    <Button
                        size="sm"
                        onClick={() => router.push(`/invoices/new?fromRecurring=${id}`)}
                    >
