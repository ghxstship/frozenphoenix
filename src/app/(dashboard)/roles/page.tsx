...
const STATIC_ROLE_META: Record<RoleKey, { label: string; description: string; color: string }> = {
    exec: { label: "Executive", description: "Full platform access with manage-level permissions across all resources", color: "bg-primary/10 text-primary" },
    pm: { label: "Project Manager", description: "Read/write access to projects, tasks, crew, scheduling, and production resources", color: "bg-info/10 text-info" },
    client: { label: "Client", description: "Read-only access to assigned projects, proposals, contracts, and approvals", color: "bg-success/10 text-success" },
    vendor: { label: "Vendor", description: "Task-specific access to assigned work, schedules, and relevant documents", color: "bg-warning/10 text-warning" },
};
...
export default function RolesPage() {
    ...
    const roles = useDbRoles
        ? dbRoles.map((r) => ({
            ...
            color: STATIC_ROLE_META[r.key as RoleKey]?.color ?? "bg-secondary/10 text-muted-foreground",
    ...
    const allResources = useDbRoles
        ...
        : [...new Set(Object.values(PERMISSION_MATRIX).flatMap((perms) => perms.map((p) => p.resource)))].sort();
                ...
                {dbLoading ? (
                    ...
                    <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <StatCard title="Permission Rules" value={totalPermissions} icon={Settings} />
                            <StatCard title="System Roles" value={roles.filter((r) => r.isSystem).length} icon={Users} />
                        </div>
                                    ...
                                    return (
                                        ...
                                        >
                                            <CardContent className="py-4">
                                                <div className={`h-10 w-10 rounded-xl flex items-center justify-center mb-3 ${role.color}`}>
                                                    <Shield className="h-5 w-5" />
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <h3 className="text-sm font-semibold">{role.label}</h3>
                                                    {role.isSystem && <Badge variant="ghost" className="text-[10px]">system</Badge>}
                                                </div>
                                                <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{role.description}</p>
                                                <div className="flex items-center justify-between mt-3">
                                                    <Badge variant="ghost">{role.permissionCount} permissions</Badge>
                                                </div>
                                            </CardContent>
                                        </Card>
                                    );
                                    ...
                                    <table className="w-full text-sm">
                                        ...
                                        <tbody>
                                            {allResources.map((resource) => {
                                                const actions = permsByResource.get(resource) ?? new Set();
                                                ...
                                                return (
                                                    <tr key={resource} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                                                        <td className="py-2.5 pr-4">
                                                            <span className="font-medium text-xs">{getStatusLabel(resource)}</span>
                                                        </td>
                                                        {(["read", "write", "delete", "manage"] as const).map((action) => {
                                                            const has = hasWildcard || actions.has(action);
                                                            return (
