...
export default function ServiceRequestsPage() {
    ...
    const inProgress = requests.filter(r => ["acknowledged", "assessment_scheduled", "quoted"].includes(r.status)).length;
                                ...
                                <div className="flex items-start gap-4">
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            <span className={`text-[10px] font-medium ${PRIORITY_CONFIG[req.priority].color}`}>
                                                {PRIORITY_CONFIG[req.priority].label}
                                            </span>
                                            <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                                                <SourceIcon className="h-3 w-3" />
                                                {SOURCE_LABELS[req.source]?.label || req.source}
                                            </span>
                                        </div>

                                        <h3 className="text-sm font-bold mb-1">{req.title}</h3>

                                        <div className="flex items-center gap-3 text-xs text-muted-foreground mb-2">
                                            {(req.companyName || req.contactName || req.requesterName) && (
                                                <span className="flex items-center gap-1">
                                                    <User className="h-3 w-3" />
                                                    {req.companyName && <span className="font-medium">{req.companyName}</span>}
                                                    {req.contactName && <span>{req.contactName}</span>}
                                                    {!req.contactName && req.requesterName && <span>{req.requesterName}</span>}
                                                </span>
                                            )}
                                            {req.requesterEmail && (
                                                <span className="flex items-center gap-1">
                                                    <Mail className="h-3 w-3" />{req.requesterEmail}
                                                </span>
                                            )}
                                            {req.preferredDate && (
                                                <span className="flex items-center gap-1">
                                                    <Calendar className="h-3 w-3" />Preferred: {req.preferredDate}
                                        ...
                                        <div className="flex items-center gap-3 text-[10px]">
                                            {req.category && (
                                                <span className="px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground">{req.category}</span>
                                            )}
                                            {req.serviceType && (
                                                <span className="px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground">{req.serviceType}</span>
                                            )}
                                            {req.requiresAssessment && (
                                                <span className="text-warning">Requires assessment</span>
                                            )}
                                            {req.assignedToName && (
                                                <span className="text-muted-foreground">Assigned: {req.assignedToName}</span>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="shrink-0 flex flex-col items-end gap-2">
                                        ...
                                        {req.status === "new" && (
                                            <Button size="sm" variant="outline" className="text-xs">Triage</Button>
                                        )}
                                        {req.status === "acknowledged" && (
                                            <Button size="sm" className="text-xs">Create Quote</Button>
                                        )}
                                        {req.status === "assessment_scheduled" && (
                                            <Button size="sm" variant="outline" className="text-xs">View Assessment</Button>
                                        )}
                                    </div>
                                </div>
                            </CardContent>
