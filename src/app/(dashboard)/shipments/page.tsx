...
export default function ShipmentsPage() {
    ...
    const filteredShipments = shipments.filter((shipment) => {
        const matchesSearch =
            shipment.number.toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        <PermissionGate resource="shipments" action="read">
            ...
            >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    ...
                    <StatCard
                        title="Total Pieces"
                        value={shipments.reduce((sum, s) => sum + (s.totalPieces || 0), 0)}
                        icon={Package}
                    />
                    <StatCard
                        title="Total Cost"
                        value={formatCurrency(shipments.reduce((sum, s) => sum + (s.cost || 0), 0))}
                        icon={Truck}
                    />
                </div>
                        ...
                        {filteredShipments.map((shipment) => {
                            const statusConfig =
                                SHIPMENT_STATUS_CONFIG[
                                    shipment.status as keyof typeof SHIPMENT_STATUS_CONFIG
                                ];
                            const typeConfig =
                                SHIPMENT_TYPE_CONFIG[
                                    shipment.type as keyof typeof SHIPMENT_TYPE_CONFIG
                                ];
                            const originLocation = locations.find(
                                (l) => l.id === shipment.originLocationId
                            );
                            const destLocation = locations.find(
                                (l) => l.id === shipment.destinationLocationId
                            );
