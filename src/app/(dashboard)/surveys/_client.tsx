...
function SurveysContent({
    ...
    enrichedTemplates: SurveyTemplate[];
                    ...
                    {tabId === "templates" &&
                        (filteredTemplates.length === 0 ? (
                            <EmptyState
                                ...
                                description={
                                    search ? "Try adjusting your search" : "No survey templates yet"
                                }
                            />
                        ) : (
                    ...
                    {tabId === "responses" &&
                        (filteredResponses.length === 0 ? (
                            <EmptyState
                                ...
                                description={
                                    search ? "Try adjusting your search" : "No survey responses yet"
                                }
                            />
                        ) : (
                                        ...
                                        {[
                                            ...
                                            >
                                                ...
                                                <span className="text-sm font-bold">
                                                    {seg.count} (
                                                    {Math.round(
                                                        (seg.count / (responses.length || 1)) * 100
                                                    )}
                                                    %)
                                                </span>
                                            </div>
...
// ─── Page ────────────────────────────────────────────────────
export function SurveysPageClient() {
        ...
        () => ({
            ...
            stats: [
                ...
                },
                {
                    ...
                    icon: ClipboardList,
                    compute: () => enrichedTemplates.filter((t) => t.isActive).length,
                },
            ],
            contentSlot: (
                <SurveysContent enrichedTemplates={enrichedTemplates} responses={responses} />
            ),
        }),
        [enrichedTemplates, responses, avgRating, promoters]
    );

    return (
        <ListPageShell
            config={config}
            data={enrichedTemplates as unknown as Record<string, unknown>[]}
            isLoading={isLoading}
        />
    );
}
