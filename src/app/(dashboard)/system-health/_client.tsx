...
export function SystemHealthPageClient() {
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="space-y-3">
                            {services.length === 0 && <EmptyRow message="No services configured" />}
    ...
    const config: DashboardPageConfig = {
        ...
        action: "read",
        title: "System Health",
        description: "Real-time monitoring, SLA tracking, and resilience metrics",
        stats: [
            {
                label: "Services",
                icon: Activity,
                compute: () => `${healthyCount}/${services.length}`,
            },
            { label: "Avg Latency", icon: Gauge, compute: () => `${avgLatency}ms` },
            { label: "Uptime", icon: HardDrive, compute: () => `${avgUptime}%` },
            { label: "Active Alerts", icon: Bell, compute: () => activeAlerts },
        ],
        contentSlot,
    };
