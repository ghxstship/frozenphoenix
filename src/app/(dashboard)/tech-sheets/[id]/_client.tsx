...
export function TechSheetDetailClient({
    id,
    initialRecord,
}: {
    id: string;
    ...
    const { data: sbRecord, isLoading } = useTechSheet(id);
    const ts = (sbRecord ?? initialRecord) as Record<string, unknown> | null;
        ...
        <DetailPageShell
            ...
            menuItems={[
                { label: "Download PDF", onClick: () => window.print() },
                {
                    ...
                    onClick: () => router.push(`/tech-sheets/new?duplicateFrom=${id}`),
                },
                ...crudMenuItems,
            ]}
            avatar={
