                ...
                {filtered.map((review, i) => (
                    <Card key={review.id} className="animate-slide-up hover:shadow-md transition-shadow" style={{ animationDelay: `${i * 60}ms` }}>
                        <CardContent className="pt-4">
                            <div className="flex items-start justify-between mb-3">
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        <h3 className="text-sm font-bold">{review.vendorName}</h3>
                                        <Badge variant={REVIEW_TYPE_LABELS[review.reviewType].variant} className="text-[10px]">
                                            {REVIEW_TYPE_LABELS[review.reviewType].label}
                                        </Badge>
                                    </div>
                                    {review.projectName && (
                                        <p className="text-xs text-muted-foreground">Project: {review.projectName}</p>
                                    )}
                                </div>
                                <div className="text-right">
                                    <StarRating rating={review.overallRating} size="md" />
                                    <p className="text-xs text-muted-foreground mt-1">{review.reviewDate}</p>
                                </div>
                            </div>
                            ...
                            {(review.strengths || review.improvements || review.comments) && (
                                <div className="space-y-2 text-xs border-t border-border pt-3">
                                    ...
                                    {review.improvements && (
                                        <div>
                                            <span className="font-medium text-warning">Improvements:</span>{" "}
                            ...
                            <div className="flex items-center justify-between mt-3 pt-2 border-t border-border text-xs">
                                <span className="text-muted-foreground">Reviewed by {review.reviewerName}</span>
                                {review.wouldRehire !== undefined && (
                                    <div className="flex items-center gap-1">
                                        {review.wouldRehire ? (
                                            <><ThumbsUp className="h-3 w-3 text-success" /><span className="text-success">Would rehire</span></>
                                        ) : (
                                            <><ThumbsDown className="h-3 w-3 text-destructive" /><span className="text-destructive">Would not rehire</span></>
                                        )}
                                    </div>
                                )}
                            </div>
