...
export default function WorkforceReviewsPage() {
    ...
    const avgRating = reviews.length > 0 ? (reviews.reduce((s, r) => s + r.overallRating, 0) / reviews.length).toFixed(1) : "0";
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <PageHeader title="Performance Reviews" description="Universal review system for all worker classifications — employees, contractors, freelancers, and vendors">
                        ...
                        <CardContent className="pt-4">
                            <div className="flex items-start justify-between mb-3">
                                ...
                                <div className="text-right">
                                    ...
                                    <p className="text-[10px] text-muted-foreground mt-0.5">{new Date(review.reviewDate).toLocaleDateString()}</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-1 mb-3">
                                ...
                                <RatingRow label="Professionalism" value={review.professionalismRating} />
                            ...
                            {review.strengths && (
                                <div className="mb-2">
                                    <p className="text-[10px] font-medium text-muted-foreground mb-0.5">Strengths</p>
                                    <p className="text-xs line-clamp-2">{review.strengths}</p>
                                </div>
                            )}
                            {review.areasForImprovement && (
                                <div className="mb-2">
                                    <p className="text-[10px] font-medium text-muted-foreground mb-0.5">Areas for Improvement</p>
                                    <p className="text-xs line-clamp-2">{review.areasForImprovement}</p>
                                </div>
                            )}
                            {review.goals && (
                                <div className="mb-2">
                            ...
                            <div className="flex items-center justify-between mt-3 pt-2 border-t">
                                <span className="text-[10px] text-muted-foreground">By {review.reviewerName}</span>
                                <div className="flex items-center gap-2">
                                    {review.wouldReengage !== undefined && (
                                        <Badge variant={review.wouldReengage ? "success" : "destructive"} className="text-[10px]">
                                            {review.wouldReengage ? "Would Re-engage" : "Would Not Re-engage"}
                                        </Badge>
                                    )}
                                    {review.acknowledgedAt ? (
                                        <span className="text-[10px] text-success">Acknowledged</span>
                                    ) : (
                                        <span className="text-[10px] text-warning">Pending Ack</span>
                                    )}
                                </div>
                            </div>
                        </CardContent>
