...
export const GET = withApiHandler(
    ...
    async (_request, { user }) => {
        ...
        const { data: memberships, error: memErr } = await serverFromTable(
            ...
            .select(
                "conversation_id, last_read_at, is_muted, is_pinned, role, notification_preference"
            )
            .eq("user_id", user.id);
        ...
        const { data: conversations, error: convErr } = await serverFromTable(
            admin!,
            "conversations"
        )
            .select(
...
export const POST = withApiHandler(
    ...
    async (request, { user, log }) => {
        ...
        if (convErr) {
            ...
            log.error("[POST /api/conversations] insert failed", { error: convErr });
        ...
        if (memErr) {
            log.error("[POST /api/conversations] members insert failed", { error: memErr });
        }
