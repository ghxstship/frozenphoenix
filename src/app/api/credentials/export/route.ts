...
export async function POST(request: NextRequest) {
    ...
    let columnMapping: { source_field: string; target_header: string; include: boolean }[] | null = null;
    if (template_id) {
        ...
        if (template) {
            columnMapping = (template as unknown as Record<string, unknown>).column_mapping as { source_field: string; target_header: string; include: boolean }[];
        }
    }
    ...
    switch (entity_type) {
        case "credential_assignments":
            query = serverFromTable(sb, "credential_assignments")
                .select("*, credential_types:credential_type_id(name, category)");
            break;
        case "credential_scan_log":
            query = serverFromTable(sb, "credential_scan_log")
                .select("*");
            break;
        case "pos_transactions":
            query = serverFromTable(sb, "pos_transactions")
                .select("*");
            break;
        default:
    ...
    if (error) {
        logger.error("[POST /api/credentials/export]", { error });
