...
export function DropdownMenuContent({
    ...
    return (
        <div
            ...
            onKeyDown={handleKeyDown}
            className={cn(
                "absolute z-50 min-w-[8rem] overflow-hidden rounded-md border bg-popover p-1 text-popover-foreground shadow-md motion-safe:animate-scale-in",
                align === "end"
                    ? "right-0"
                    : align === "start"
                      ? "left-0"
...
export function DropdownMenuItem({ children, className, onClick }: DropdownMenuItemProps) {
    ...
    return (
        <button
            ...
            tabIndex={-1}
            className={cn(
                "relative flex w-full cursor-default select-none items-center rounded-sm px-2 py-1.5 min-h-[44px] text-sm outline-none transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground",
                className
            )}
            onClick={() => {
                onClick?.();
